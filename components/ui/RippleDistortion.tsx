...
const RippleDistortion: React.FC<RippleDistortionProps> = ({
  ...
  useEffect(() => {
    ...
    (waveProgram as any).setBlendFunc(gl.ONE, gl.ONE);
    ...
    const compositeMesh = new Mesh(gl, {
      ...
      program: new Program(gl, {
        ...
        uniforms: compositeUniforms as Record<string, { value: unknown }>,
        depthTest: false,
        depthWrite: false,
      }),
    });
    ...
    uniformsRef.current = {
      wave: waveUniforms,
      composite: compositeUniforms as Record<string, { value: unknown }>,
    };
  ...
  useEffect(() => {
    ...
    (u.composite.uStrength as { value: number }).value = strength;
    (u.composite.uSwirl as { value: number }).value = swirl;
    (u.composite.uDispersion as { value: number }).value = dispersion;
    (u.composite.uGlint as { value: number }).value = glint;
    (u.composite.uTintAmount as { value: number }).value = tintAmount;
